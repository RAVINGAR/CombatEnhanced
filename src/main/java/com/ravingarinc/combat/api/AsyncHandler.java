...
public class AsyncHandler {
    private static CombatEnhanced plugin;
    private static BukkitScheduler scheduler;

    private AsyncHandler() {
    }

    public static void load(final CombatEnhanced plugin) {
    ...
    public static <V> void executeAsyncComputation(final Callable<V> callable, final Consumer<V> consumer) {
        scheduler.runTaskAsynchronously(plugin, () -> {
            try {
                ...
                CombatEnhanced.log(Level.SEVERE, "Encountered issue running async computation!", e);
            }
        });
    }
