...
public class PoiseRunner extends BukkitRunnable {
    ...
    public void handle(final HeroesDamageEvent event) {
        ...
        if(character.hasEffect(PoiseBreakEffect.EFFECT_NAME)) {
            return;
        }
        ...
        final var impact = (event.getAttacker() instanceof Hero hero) ? MMOPlayerData.get(hero.getPlayer()).getStatMap().getStat(KalentireHandler.IMPACT) : 0.0;
        ...
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            ...
            final var damageOverPoise = poiseDamage - (handler.getPoise(entity));
            if(damageOverPoise <= 0) {
                return; // Still below poise
            }
            character.addEffect(new PoiseBreakEffect((int)(damageOverPoise / settings.stunThreshold) * settings.stunDurationPerThreshold, settings.stunCooldown, damageOverPoise * settings.vulnerabilityPerPoise));
